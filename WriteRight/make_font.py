...
class FontMaker:
    ...
    def create_font_file(self, character_images, font_name, output_dir):
        """创建字体文件"""
        # 合并所有常用字符
        all_chars = (self.common_chars['letters'] +
                     self.common_chars['numbers'] +
                     self.common_chars['symbols'])

        # 创建字体数据
        font_data = self.build_font_data(character_images, all_chars)
